...
function media () {
  ...
  stream = stream.pipe(global.useSymlinks ? symlink(paths.media.destinationPath, { overwrite: true }) : dest(paths.media.destinationPath))

  stream = stream.on('finish', () => {
    console.log('     ', global.mediaCounter, 'media files', global.useSymlinks ? 'symlinked' : 'copied')
  })
...
function optimizeMedia () {
  ...
  stream = stream.pipe(dest(paths.media.sourcePath))
